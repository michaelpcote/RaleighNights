<?php
	...
	$conn =  Common::getConn();
	$query_firm = "INSERT INTO firm ( firm_type, name, phone, address, city, state, zip, latitude, longitude ) VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ? )";
	...
	if ($stmt = mysqli_prepare($conn, $query_firm) ) {
		$stmt->bind_param("issssssss", $_POST['firm_type'], $_POST['company'], $_POST['bus_phone'], $_POST['address'], $_POST['city'], 
																				$_POST['state'], $_POST['zip'], $output[0], $output[1]);
		$stmt->execute();
