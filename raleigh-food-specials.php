...
<title>Raleigh Food Specials</title>
...
<body>
    	...
    	<div class="innerBox">
        	<div class="contentTitle">Raleigh Food Specials</div><br />
            <?php
            $conn =  Common::getConn();
			$day = strtolower( date('l'));
			$query = "SELECT f.firm_id, f.name, f.website, f.address, f.city, f.state, f.zip, ds.long_description, ds.short_description FROM firm f, ".$day."_food_specials ds ";
			$query .= "WHERE f.verified = 1 AND f.firm_id = ds.firm_id ORDER BY f.name";
			$result = $conn->query($query);
			?>
          <div class="contentText">
          <p>Raleigh Nights is pleased to present you with a list of Raleigh food specials! These are the food specials in Raleigh for today, but we also have the food
          specials for every other day of the week. If you want to see a particular day please select
          from the food specials drop down menu. If you are a bar owner and you don't see your special on here, please log
