...
<title>Edit Raleigh Events</title>
...
<body>
    	...
    	<div class="innerBox">
        	<div class="contentTitle">Events in Raleigh Edit</div><br />
            <?php
			...
			$query .= "LEFT OUTER JOIN monday_events mon ON f.firm_id = mon.firm_id ";
			$query .= "LEFT OUTER JOIN tuesday_events tues ON f.firm_id = tues.firm_id ";
			$query .= "LEFT OUTER JOIN wednesday_events wed ON f.firm_id = wed.firm_id ";
			$query .= "LEFT OUTER JOIN thursday_events thurs ON f.firm_id = thurs.firm_id ";
			$query .= "LEFT OUTER JOIN friday_events fri ON f.firm_id = fri.firm_id ";
			$query .= "LEFT OUTER JOIN saturday_events sat ON f.firm_id = sat.firm_id ";
			$query .= "LEFT OUTER JOIN sunday_events sun ON f.firm_id = sun.firm_id ";
			$query .= "WHERE f.firm_id = ".$_SESSION['firm_id'];
			$result = $conn->query($query);
			?>
          <div class="contentText">
          <p>This page will enable you to edit the drink specials. If this is not the restuarnt or bar that you would like to events drink specials for, please
          <a href="new-restaurant-or-bar.php">choose a new restaurant or bar here</a></p><br />
          
          <p>The events listed here should be reoccurring events. If for example, you have trivia night every Monday and a Karaoke night every Thursday, those events should be listed here. If you have a special event upcoming, like a band, please add <a href=”admin-add-special-event.php”>your special event here.</a> 
          <?php 
        ...
        <form action="admin-edit-specials-process.php" method="post" id="delete">
        	<fieldset>
                <legend>Edit events for <?php echo $name ?></legend>
                <div class="field">
         ...
         <br />
         <?php } else { ?>
         	<p>You haven't selected a restaurant or bar to edit. Please <a href="admin-restaurant-and-bar-lists.php">select a bar here</a></p>
         <?php } ?>
